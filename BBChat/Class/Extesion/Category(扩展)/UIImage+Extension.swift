...
extension UIImage {
    ...
    /**
         根据颜色返回一张含线性渐变的图片(颜色暂时固定)
         - parameter size: 大小
     */
    public static func imageWithLinearGradientColors(size: CGSize = CGSize.zero) -> UIImage? {
        var size = size
        if size == CGSize.zero {
            size = CGSize(width: MGScreenW, height: 64)
        }
        ...
        let compoents: [CGFloat] = [255/255, 227/255, 122/255, 1, 255/255, 218/255, 80/255, 1, 255/255, 201/255, 0/255, 1,]
        // 没组颜色所在位置（范围0~1)
...
extension UIImage {
    ...
    func imageWithTitle(title: String, drawAtPoint: CGPoint, fontSize: CGFloat) -> UIImage? {
        ...
        paragraphStyle.alignment = .center//文字居中
        
        //计算文字所占的size,文字居中显示在画布上
        let sizeText = title.boundingRect(with: self.size, options: .usesLineFragmentOrigin, attributes: [.font: UIFont.systemFont(ofSize: fontSize)] , context: nil);
        ...
        //绘制文字
        title.draw(in: rect, withAttributes: [.font: UIFont.systemFont(ofSize: fontSize), .foregroundColor: UIColor.white, .paragraphStyle: paragraphStyle])
