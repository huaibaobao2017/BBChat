...
//        let maskPath = UIBezierPath(roundedRect: self.frame, cornerRadius: cornerRadius)
     ...
     *  这时候可以使用响应者链直接拿到父控制器，避免使用多重block嵌套或者通知这种情况
     */
...
// MARK: - 单独设置view的边框
extension UIView {
    static func borderWithView(view: UIView, top: Bool? = false, left: Bool? = false, bottom: Bool? = false, right: Bool? = false, color: UIColor? = UIColor.lightGray, width: CGFloat? = 0.5, offset: CGFloat? = 0) -> CALayer {
        let layer = CALayer()
        layer.backgroundColor = color?.cgColor
        if top! {
            layer.frame = CGRect(x: 0, y: offset!, width: view.frame.size.width, height: width!)
        }
        if left! {
            layer.frame = CGRect(x: offset!, y: 0, width: width!, height: view.frame.size.height)
        }
        if bottom! {
            layer.frame = CGRect(x: 0, y: view.frame.size.height - width! + offset!, width: view.frame.size.width, height: width!)
        }
        if right! {
            layer.frame = CGRect(x: view.frame.size.width - width! + offset!, y: 0, width: width!, height: view.frame.size.height)
        }
        return layer
    }
}
