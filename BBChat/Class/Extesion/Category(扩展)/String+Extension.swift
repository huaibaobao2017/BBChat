...
extension String {
    ...
    func getSpaceLabelHeight(with font: UIFont, withWidth width: CGFloat) -> CGFloat {
        ...
        let dic: [AnyHashable: Any] = [NSAttributedStringKey.font: font, NSAttributedStringKey.paragraphStyle: paraStyle, NSAttributedStringKey.kern: 1.5]
        let size = self.boundingRect(with: CGSize(width: width, height: CGFloat(HUGE)), options: .usesLineFragmentOrigin, attributes: (dic as? [NSAttributedStringKey : Any] ?? [NSAttributedStringKey : Any]()), context: nil).size
        return size.height
    }
    ...
    /**
     ...
     - parameter Float: 返回计算好的高度
     */
    public func inputTextHeight(for width: CGFloat) -> CGFloat {
        let constraint = CGSize(width: width, height: CGFloat.greatestFiniteMagnitude)
        let size: CGRect = self.boundingRect(with: constraint, options: ([.usesLineFragmentOrigin, .usesFontLeading]), attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: 14)], context: nil)
        return size.size.height + 22.0
    }
    ...
    /**
     ...
     - parameter Float: 返回计算好的高度
     */
    public func inputTextWidth(for height: CGFloat, font: CGFloat) -> CGFloat {
        let constraint = CGSize(width: CGFloat.greatestFiniteMagnitude, height: height)
        let size: CGRect = self.boundingRect(with: constraint, options: ([.usesLineFragmentOrigin, .usesFontLeading]), attributes: [NSAttributedStringKey.font: UIFont.systemFont(ofSize: font)], context: nil)
        return size.size.width + 6
    }
